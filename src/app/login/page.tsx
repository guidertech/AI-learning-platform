...
export default function LoginPage() {
  ...
  return (
    <DeviceViewer title="ClassOrbit Onboarding" deviceType="MOBILE">
      <main className="relative h-full min-h-screen w-full flex flex-col items-center justify-between premium-gradient px-margin-page py-[60px] overflow-hidden">
        ...
        <div className="flex-grow flex flex-col items-center justify-center z-10">
          <div className="logo-float mb-gutter-stack flex flex-col items-center">
            ...
            <h1 className="font-display text-[28px] font-bold text-primary tracking-tight">ClassOrbit</h1>
            <p className="font-label-md text-xs text-on-surface-variant font-medium mt-2">Luminous Intelligence</p>
          </div>
        </div>
        ...
        <div className="w-full max-w-sm z-10 flex flex-col gap-4 fade-in-up mb-4 select-none animate-fade-in">
          <button
            ...
            disabled={loading}
            className="h-12 w-full bg-white rounded-full flex items-center justify-center gap-3 border border-outline-variant/30 ambient-bloom hover:bg-surface-container-low transition-all duration-300 active:scale-95 shadow-[0_4px_12px_rgba(0,0,0,0.05)] cursor-pointer"
          >
            ...
            <span className="font-label-md text-xs font-bold text-on-surface">
              {loading ? "Connecting to Supabase..." : "Continue with Google"}
            </span>
          </button>
          <p className="text-center text-[9px] text-on-surface-variant/60 px-4 leading-relaxed">
            By continuing, you agree to ClassOrbit's Terms of Service and Privacy Policy.
          </p>
        </div>
      </main>
    </DeviceViewer>
