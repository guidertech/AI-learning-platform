...
export default function PrerequisiteQuizPage({ params }: PrerequisitePageProps) {
  ...
  const questions = quizData?.prerequisite ?? [];
  ...
  if (!quizData || totalQuestions === 0) {
    return (
      <PageContainer>
        <Topbar title="Prerequisite Test" />
        <main className="p-8">
          <EmptyState title="No Quiz Found" description="No prerequisite quiz is available for this chapter." icon="quiz" />
        </main>
      </PageContainer>
    );
  }
