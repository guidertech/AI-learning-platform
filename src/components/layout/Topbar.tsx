...
export default function Topbar({ title, subtitle, showBack = false }: TopbarProps) {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-md flex justify-between items-center gap-4 w-full px-4 md:px-8 h-16 md:h-20 border-b border-outline-variant/10 select-none">
      <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-4">
        {/* Brand Logo inside Topbar */}
        <div className={`flex items-center gap-2 shrink-0 ${showBack ? "hidden sm:flex" : ""}`}>
        ...
        {/* Brand Divider */}
        <div className={`h-5 w-px bg-slate-200/80 shrink-0 ${showBack ? "hidden sm:flex" : ""}`} />
        ...
        <div className="min-w-0 flex-1">
          <h1 className="font-display font-bold text-sm sm:text-base md:text-xl text-on-surface leading-tight truncate">{title}</h1>
          {subtitle && <p className="text-[10px] md:text-[11px] text-on-surface-variant font-medium mt-0.5 hidden xs:block truncate">{subtitle}</p>}
        </div>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-1.5 sm:gap-3">
        ...
        <div className="flex items-center gap-1 px-1.5 sm:px-2.5 py-1 bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded-full font-black text-[10px] uppercase tracking-wider shadow-2xs select-none border border-indigo-400/20">
          <span className="material-symbols-outlined text-[13px]" style={{ fontVariationSettings: "'FILL' 1" }}>stars</span>
          <span className="hidden xs:inline">Level {studentLevel}</span>
          <span className="xs:hidden">L{studentLevel}</span>
        </div>
