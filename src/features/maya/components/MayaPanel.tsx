...
import AITeacherCard from "./AITeacherCard";
import AskMayaInput from "./AskMayaInput";
              ...
              {chatMessages.slice(1).map((msg) => {
                ...
                return (
                  <div key={msg.id} className={`flex gap-3 items-start ${isAI ? "" : "flex-row-reverse"}`}>
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 shadow-sm ${isAI ? "bg-primary-container text-white" : "bg-secondary text-white"
                      }`}>
                      <span className="material-symbols-outlined text-[16px]">{isAI ? "psychology" : "person"}</span>
                    </div>
                    <div className={`p-4 rounded-2xl border text-xs max-w-[80%] leading-relaxed ${isAI
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              {chatMessages.slice(1).map((msg) => {
                ...
                return (
                  <div key={msg.id} className={`flex gap-3 items-start ${isAI ? "" : "flex-row-reverse"}`}>
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 shadow-sm ${isAI ? "bg-primary-container text-white" : "bg-secondary text-white"
                      }`}>
